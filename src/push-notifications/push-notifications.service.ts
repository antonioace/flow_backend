...
export class PushNotificationsService implements OnModuleInit {
    ...
    async sendNotificationToUser(userId: string, payload: any) {
        ...
        this.logger.log(`Sending notification to user ${userId} with ${subscriptions.length} subscriptions`);
        const results = await Promise.all(subscriptions.map(async sub => {
            ...
            if (!success) {
                this.logger.warn(`Failed to send push to ${sub.endpoint}, removing subscription`);
